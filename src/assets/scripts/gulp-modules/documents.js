document.querySelectorAll('.doc:not(:last-child)').forEach(element => {
    element.addEventListener('mouseenter', function(evt) {
        element.nextElementSibling.style.borderTopColor = `var(--orange)`;
    });
    element.addEventListener('mouseleave', function(evt) {

        element.nextElementSibling.style.borderTopColor = ``;
    });
});
